...
% Similar to pen2_dumb, but passes around the initial lists as args
% Like a lambda-lifted version of pen2?
pen2_dumb_opt(N) ->
    Lst = lists:reverse(lists:seq(1,N)),
    pen2_dumb_opt_helper(Lst, Lst, Lst, Lst, []).
pen2_dumb_opt_helper([], _, _, _, L) -> L;
...
pen2_dumb_opt_helper([X|Xs], _, XGen, YGen, L) when X rem 2 /= 0 ->
    pen2_dumb_opt_helper(Xs, YGen, XGen, YGen, L);
pen2_dumb_opt_helper(AX = [X|_Xs], [Y|Ys], XGen, YGen, L) when Y rem 2 =:= 1 andalso
                                                      (X+Y) rem 5 /= 0 ->
...
nested_lc_gen_opt(Lst) ->
    Entries = lists:reverse([ Val || Val <- Lst, Val rem 2 =:= 1]),
    nested_lc_gen_opt_helper(Entries, []).
nested_lc_gen_opt_helper([Entry|Entries], Res) ->
    nested_lc_gen_opt_helper(Entries, [#test_record{num1 = Entry, num2 = Entry * Entry}|Res]);
nested_lc_gen_opt_helper([], Res) -> Res.
